...
import { AboutForm, staticPagesService } from "@/features/static-pages";
...
export default function AboutEditPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["about-page"],
    queryFn: staticPagesService.getAbout,
    staleTime: 5 * 60 * 1000,
  });
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-start gap-4">
          ...
          <PageTitle title={t("about.title")} description="" />
        </div>
    ...
    <div className="h-full overflow-y-scroll pr-2">
      <div className="max-w-4xl pb-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <AboutForm initialData={data} locale={locale} />
        </div>
      </div>
    </div>
  );
