...
import type { ITermsPage, TermsFormValues } from "../types";

interface TermsFormProps {
  initialData?: ITermsPage;
  locale: "fr" | "en";
}
...
export function TermsForm({ initialData, locale }: TermsFormProps) {
  ...
  const isCreateMode = !initialData || initialData.id === "default-terms";

  // Language labels
  const langLabel = locale === "fr" ? "Français" : "English";

  const mutation = useMutation({
    mutationFn: staticPagesService.updateTerms,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["terms-page"] });
    },
    onError: (error: any) => {
  ...
  const formik = useFormik<TermsFormValues>({
    initialValues: {
      // Use content for current locale
      content: (initialData?.content?.[locale] || emptyContent) as TermsFormValues["content"],
    },
