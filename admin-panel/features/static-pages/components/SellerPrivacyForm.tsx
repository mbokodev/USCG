...
import type { ISellerPrivacyPage, SellerPrivacyFormValues } from "../types";

interface SellerPrivacyFormProps {
  initialData?: ISellerPrivacyPage;
  locale: "fr" | "en";
}
...
export function SellerPrivacyForm({ initialData, locale }: SellerPrivacyFormProps) {
  ...
  const isCreateMode = !initialData || initialData.id === "default-seller-privacy";

  const mutation = useMutation({
    mutationFn: staticPagesService.updateSellerPrivacy,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["seller-privacy-page"] });
    },
    onError: (error: any) => {
  ...
  const formik = useFormik<SellerPrivacyFormValues>({
    initialValues: {
      // Use content for current locale
      content: (initialData?.content?.[locale] || emptyContent) as SellerPrivacyFormValues["content"],
    },
